...
static void bucket_sort(int n, int x[n], int m, int idx[m], int offset,
    ...
    memcpy(idx, sorted, m * sizeof(*sorted));
...
static int *build_alphabet(int n, int x[n], int sa12[], int *new_asize) {
    ...
    assert(sa12len(n) > 0); // help the static analyser
...
int *cstr_skew(char const *x) {
    enum cstr_errcodes err; // FIXME: handle errors
    
    struct cstr_str_slice slice = cstr_slice_from_string((char *)x);
    struct cstr_alphabet *alpha =
        cstr_alphabet_from_slice(slice, &err);
    int *arr =
        cstr_alphabet_map_to_int(alpha, slice, &err);

    int *sa = skew_rec(slice.len + 1, arr, alpha->size);
