...
struct inner_node_pool
{
    size_t block_size;
    node_id next;
    struct inner node_blocks[]; // type struct inner for align; use char * for addresses
};
...
static struct inner_node_pool *new_pool(long long sigma, long long n)
{
    ...
    // Now, allocate a pool of n - 1 of those blocks.
...
static inner *pool_get(struct inner_node_pool *pool, node_id n)
{
    ...
    void *block = (char *)pool->node_blocks + offset;
...
static leaf *get_leaf(cstr_suffix_tree *st, node_id n)
{
    assert(is_leaf(n));
...
static inner *get_inner(cstr_suffix_tree *st, node_id n)
{
    assert(is_inner(n));
...
static void add_child(cstr_suffix_tree *st, node_id parent_id, node_id child_id)
{
    assert(is_inner(parent_id));
    inner *parent = get_inner(st, parent_id);
    cstr_const_sslice child_edge = get_edge(st, child_id);
    node_id existing_child = parent->children[child_edge.buf[0]];
    assert(is_null(existing_child));
...
{
    // Leaves and null doesn't have children
    if (n <= 0)
    {
        return 0;
    }
...
static long long shared_prefix(cstr_suffix_tree *st, node_id node, cstr_const_sslice p)
{
...
cstr_new_suffix_tree(cstr_alphabet const *alpha, cstr_const_sslice x)
{
...
TL_TEST(st_constructing_leaves)
{
    ...
    cstr_suffix_tree *st = cstr_new_suffix_tree(&alpha, x);
...
TL_TEST(st_constructing_inner_nodes)
{
    ...
    add_child(st, st->root, leaf_id(0));
    ...
    add_child(st, st->root, leaf_id(1));
    // and "ssissippi"
    add_child(st, st->root, leaf_id(2));
